...
```
autodiscovery-results/pilot-to-full/<experiment_id>/
...
manifest.to_csv(
    f"autodiscovery-results/pilot-to-full/<experiment_id>/session_manifest.csv",
    index=False
)
...
```bash
mkdir -p autodiscovery-results/pilot-to-full/<experiment_id>/artifacts
```
...
artifacts_dir = "autodiscovery-results/pilot-to-full/<experiment_id>/artifacts"
...
py_path = "autodiscovery-results/pilot-to-full/<experiment_id>/pilot_to_full.py"
with open(py_path, "w") as f:
    f.write(notebook_py)
```

```bash
jupytext --to ipynb autodiscovery-results/pilot-to-full/<experiment_id>/pilot_to_full.py
rm autodiscovery-results/pilot-to-full/<experiment_id>/pilot_to_full.py
```
...
python3 -m nbconvert --to notebook --execute \
    ...
    --output-dir autodiscovery-results/pilot-to-full/<experiment_id>/ \
    autodiscovery-results/pilot-to-full/<experiment_id>/pilot_to_full.ipynb
```
...
Save to `autodiscovery-results/pilot-to-full/<experiment_id>/replication_report.md`.
